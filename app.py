...
def get_rest_result(title, time_line, time_line_enemy, time_data, total_damage, damages, debuff_value):
    rest_result = {"title": title, "timeline": time_line, "timeline_enemy": time_line_enemy, "damages": damages, "process_time": time_data,
    ...
    if time_line:
        ...
        if damages:
            rest_result["timeline_txt_damages"] = "\r\n".join(list(
                map(lambda x: "{} {} {}".format(x[1].split()[0], str(damages[x[0]][0:]).rjust(8, " "), x[1].split()[1]),
                    enumerate(time_line))))
        else:
            rest_result["timeline_txt_damages"] = False
...
def index():
    if request.method == "POST":
        ...
        if url_result % 100 // 10 == 2:
            ...
            cm.save_cache(youtube_id, title, False, False, False, False, False, False, url_result)
            return render_template("index.html", error=error)
...
def analyze():
    ...
    if request.method == "GET" and path is not None:
        ...
        # キャッシュ保存
        status = cm.save_cache(youtube_id, title, time_line, time_line_enemy, False, total_damage, debuff_value, damage_list, status)
