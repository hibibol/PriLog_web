...
def save_cache(youtube_id, title, time_line, time_line_enemy, time_data, total_damage, debuff_value, damages, status):
    """save cache
    ...
    if past_status is False:
        ...
        json.dump([title, time_line, time_line_enemy, time_data, total_damage, debuff_value, damages, present_status],
...
def cache_check(youtube_id):
    ...
    try:
        ...
        if len(ret) is CACHE_ELMS:  # in case of number of cached elements is correct
            title, time_line, time_line_enemy, time_data, total_damage, debuff_value, damages, past_status = ret
...
def watchdog(youtube_id, is_parent, margin, err_type):
    ...
    if os.path.exists(job_path):
        if check_pass_time(job_path, margin):
            save_cache(youtube_id, "", False, False, False, False, False, False, err_type)
            clear_path(job_path)
            if is_parent:
                clear_path(queue_path)
